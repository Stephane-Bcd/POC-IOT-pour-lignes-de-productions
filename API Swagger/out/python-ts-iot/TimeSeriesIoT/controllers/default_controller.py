...
#docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' mongodb_mongo_1

def connect_to_collection(address="172.19.0.4", port="27017", db="Client1", col="captures"):
    myclient = pymongo.MongoClient("mongodb://"+address+":"+port+"/")
    mydb = myclient[db]
    mycol = mydb[col]
    return mycol

def get_sensor_captures(mycol=connect_to_collection(), sensor_name="Temperature0"):
...
def get_last_sensor_capture(mycol=connect_to_collection(), sensor_name="Temperature0"):
    myquery = {"NomCapteur": sensor_name}
...
def get_avg_in_period(mycol=connect_to_collection(), sensor_name="Temperature0", start=datetime(2019, 9, 1, 00, 00, 00), end=datetime(2020, 1, 1, 00, 00, 00)):
...
def get_min_in_period(mycol=connect_to_collection(), sensor_name="Temperature0", start=datetime(2018, 9, 1, 00, 00, 00), end=datetime(2020, 1, 1, 00, 00, 00)):
